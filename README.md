...
### 파일을 활용한 데이터 보관
 - 파일에 데이터를 기록하여 데이터를 영구적으로 저장할 수 있음
 - 파이썬은 이런 파일을 쉽게 다루기 위해 편리한 '파일 객체'를 제공
 - 파이썬 기본 내장 함수인 open() 함수에 파일을 저장할 경로와 읽기/쓰기 모드를 설정하여 파일 객체를 얻을 수 있음
...
```
 - open() 함수의 첫 번째 인자 값으로 파일 경로, 파일 경로는 절대 경로와 상대 경로를 모두 지원
 - 두 번째 인자 값에는 읽기/쓰기 모드가 들어가고, 'w'의 의미는 'write'의 약자로 파일에 데이터를 쓰는 용도로 열겠다는 의미

|모드|설명|
|--|--|
|'r'|read. 읽기 모드 (기본 설정)|
|'w'|write. 쓰기 모드. 기존 파일 내용은 지우고 덮어 씀|
|'x'|execlusive creation. 파일이 이미 존재하면 실패함|
|'a'|appending. 기존 파일의 끝부분부터 내용이 추가됨|
|'b'|binary mode. 텍스트가 아닌 바이너리 코드로 읽고 씀|
|'t'|text mode. 바이너리가 아닌 텍스트로 읽고 씀 (기본 설정)|
|'+'|reading and writing. 읽고 쓰기 위함|

 - 만약 open() 함수 사용시 모드 인자에 값을 넣지 않는다면 기본 설징인 'rt'
 - 파일에 내용을 입력하려면 조금 전 획득한 객체에 write() 메소드를 활용
...
14
```
 - 14는 파일에 쓴 문자의 개수를 의미
 - 위 상태까지만 하면 **메모리상의 파일 객체에는 내용을 입력했지만 실제 파일에는 아직 쓰지 않은 상태**
 - flush() 메소드에 의해 실제 파일에 데이터가 입력

```python3
# 실제 파일에 데이터 입력하기
>>> f.flush()
```
 - 연 파일에 대한 모든 작업을 끝내려면 반드시 파일 객체를 닫아줘야 함
